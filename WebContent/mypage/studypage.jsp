		...
		<c:forEach var="in" items="${idAndInput }">
			...
			정답률:${in.correction/in.total*100 }%</div>
				...
				<c:choose>
					...
					<div><input type="radio" checked="checked" > ${in.s1 }</div>
					</c:when>
					<c:otherwise>
					<div><input type="radio" > ${in.s1 }</div>
					</c:otherwise>
				</c:choose>
				<c:choose>
					<c:when test="${in.input==(no.index+2) }">
					<div><input type="radio" checked="checked" > ${in.s2 }</div>
					</c:when>
					<c:otherwise>
					<div><input type="radio" > ${in.s2 }</div>
					</c:otherwise>
				</c:choose>
								<c:choose>
					<c:when test="${in.input==(no.index+3) }">
					<div><input type="radio" checked="checked" > ${in.s3 }</div>
					</c:when>
					<c:otherwise>
					<div><input type="radio" > ${in.s3 }</div>
					</c:otherwise>
				</c:choose>
				<c:choose>
					<c:when test="${in.input==(no.index+4) }">
					<div><input type="radio" checked="checked" > ${in.s4 }</div>
					</c:when>
					<c:otherwise>
					<div><input type="radio" > ${in.s4 }</div>
					</c:otherwise>
				</c:choose>
				<c:choose>
					<c:when test="${in.input==(no.index+5) }">
					<div><input type="radio" checked="checked" > ${in.s5 }</div>
					</c:when>
					<c:otherwise>
					<div><input type="radio" > ${in.s5 }</div>
					</c:otherwise>
				</c:choose>			
			</c:forEach>
